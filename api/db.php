...
function add_habit($description, $user_id) {
	...
	return get_habit_by_description($description);
}
...
function get_habit_by_description($description) {
	global $dbh;
	$stmt = $dbh->prepare("SELECT * FROM habits WHERE description = :description");
	$stmt->bindParam(':description', $description);
	$stmt->execute();
	$res = $stmt->fetch();
	$habit = new Habit($res['id'], $res['description']);
